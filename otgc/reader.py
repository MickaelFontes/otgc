...
class EventICS:
    ...
    def to_list(self):
        return (self.t_begin, self.t_end, self.location, self.summary, self.description)
...
class ReaderICS:
    ...
    def read(self):
        ...
        if self.is_path:
            with open(self.calendar, "r", encoding="UTF-8") as file:
                ics_text = file.read()
        else:
            ics_text = self.calendar
        ics_text = ReaderICS.sep.split(ics_text)
        ics_text = [
            ReaderICS.sep_fields.split(text)[1:]
